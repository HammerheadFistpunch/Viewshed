...
class ViewshedWorkspace(_ViewshedWorkspace):
    ...
    def _set_area_progress(self, elapsed: int, total: int, roles: int, positions: int, packets: int) -> None:
        ...
        status = f"Finding stations — {remaining}s remaining · {roles} infrastructure calls · {positions} position packets"
    ...
    def _restore_main_cancel_button(self) -> None:
        try:
            self.app.cancel_btn.configure(text="Cancel Run", command=self.app.cancel_job, state="normal" if getattr(self.app, "_job_running", False) else "disabled")
        except Exception:
            pass
    ...
    def find_area_stations(self) -> None:
        ...
        try:
            self.app.cancel_btn.configure(text="Cancel Step 1", command=self.cancel_area_acquisition, state="normal")
        except Exception:
            pass

        def report(elapsed, total, roles, positions, packets):
            self.after(0, lambda: self._set_area_progress(elapsed, total, roles, positions, packets))

        def work():
            try:
                cache_path = station_sources.acquire_station_cache(
                    seed_path=Path(self.app.source_var.get()), data_root=portable_data_root(), center_lat=region.center_lat,
                    center_lon=region.center_lon, acquisition_radius_km=region.radius_km + prop, refresh=True,
                    refresh_seconds=refresh_seconds, callsign=self.app.callsign_var.get().strip().upper(),
                    aprs_fi_api_key=self.app.aprsfi_var.get().strip(), stop_event=self._area_cancel_event, progress=report,
                )
                if self._area_cancel_event.is_set():
                    self.after(0, self._area_acquire_cancelled); return
                self.after(0, lambda: self._set_area_post_sample_status("Live sample complete — checking station locations…"))
                records = viewshed_core.assess_station_locations(viewshed_core.load_station_records(cache_path))
                selected = viewshed_core.filter_stations(records, region, types, prop)
                if self._area_cancel_event.is_set():
                    self.after(0, self._area_acquire_cancelled); return
                if selected:
                    self.after(0, lambda: self._set_area_post_sample_status(f"Checking {len(selected)} station locations against OpenStreetMap…"))
                    try:
    ...
    def _set_area_post_sample_status(self, status: str) -> None:
        ...
        self.area_status.set(status)
        try:
            self.app.progress["value"] = 100; self.app.percent_var.set("100%"); self.app.status_var.set(status)
        except Exception:
            pass
    ...
    def _area_acquire_cancelled(self) -> None:
        ...
        try:
            self.app.progress["value"] = 0; self.app.percent_var.set("0%"); self.app.status_var.set("Step 1 cancelled")
        except Exception:
            pass
    ...
    def _area_acquired(self, records: list[dict]) -> None:
        ...
        try:
            self.app.progress["value"] = 100; self.app.percent_var.set("100%"); self.app.status_var.set(f"Step 1 complete — {len(records)} stations ready")
        except Exception:
            pass
        self._restore_main_cancel_button()
        ...
        try:
            ...
            def append_log(text: str) -> None:
                ...
                if viewer is not None and viewer.winfo_exists():
                    viewer.configure(state="normal"); viewer.insert("end", text); viewer.see("end"); viewer.configure(state="disabled")
    ...
    def open_log_window(self) -> None:
        if self._log_window is not None and self._log_window.winfo_exists():
            self._log_window.deiconify(); self._log_window.lift(); self._log_window.focus_force(); return
        ...
        outer = ttk.Frame(window, padding=10); outer.pack(fill="both", expand=True)
        ttk.Label(outer, text="Run progress & log", font=("Segoe UI", 13, "bold")).pack(anchor="w")
        ttk.Label(outer, text="Detailed diagnostics for station acquisition and propagation runs.").pack(anchor="w", pady=(2, 8))
        body = ttk.Frame(outer); body.pack(fill="both", expand=True)
        text = tk.Text(body, wrap="word", state="normal", font=("Consolas", 9), padx=8, pady=6, relief="flat", borderwidth=0)
        scroll = ttk.Scrollbar(body, orient="vertical", command=text.yview)
        text.configure(yscrollcommand=scroll.set); text.pack(side="left", fill="both", expand=True); scroll.pack(side="right", fill="y")
        try:
            existing = self.app.log.get("1.0", "end-1c")
        except Exception:
            existing = ""
        if existing:
            text.insert("end", existing); text.see("end")
        ...
        def close() -> None:
            self._log_text = None; self._log_window = None; window.destroy()
    ...
    def _build_help_tab(self) -> None:
        ...
        ttk.Label(tab, text=("Portable map-first APRS/VHF terrain propagation analysis for the continental United States. "
                             "Signal Peak is free and open-source software licensed under GNU GPL v2 only. "
                             "The documentation below is bundled with the application for offline access."), wraplength=900).pack(anchor="w", pady=(4, 12))
        docs = ttk.LabelFrame(tab, text="Documentation", padding=10); docs.pack(fill="x")
        for index, (label, relative_path) in enumerate(self.DOCS):
            row = index // 2; col = index % 2
            ttk.Button(docs, text=label, command=lambda p=relative_path: self._open_bundled_doc(p)).grid(row=row, column=col, sticky="ew", padx=(0 if col == 0 else 6, 6 if col == 0 else 0), pady=3)
        docs.columnconfigure(0, weight=1); docs.columnconfigure(1, weight=1)
        data_box = ttk.LabelFrame(tab, text="Data and diagnostics", padding=10); data_box.pack(fill="x", pady=(12, 0))
        ttk.Label(data_box, text=(f"Application data: {portable_data_root()}\nAdvanced settings: ViewshedData/advanced_settings.json\n"
                                  ...
                                  "Jobs: ViewshedData/jobs/<timestamp>/\naprs.fi API keys are session-only and are not retained in settings.json."), wraplength=900, justify="left").pack(anchor="w")
        row = ttk.Frame(data_box); row.pack(fill="x", pady=(8, 0))
        ...
        caution = ttk.LabelFrame(tab, text="Key modeling caution", padding=10); caution.pack(fill="x", pady=(12, 0))
        ttk.Label(caution, text=("Area and Station results use explicit fallback assumptions when reliable per-station RF data is not available. "
                                 "Station Data overrides can improve those assumptions, but manually entered values are only as reliable as their source. "
                                 "The reference profile uses a 138 dB operational path-loss cap plus reduced lateral radial fill to avoid overstating marginal canyon coverage. "
                                 "Custom mode uses a 20 dB operational reserve. Coverage is a prediction, not a communications guarantee."), wraplength=900).pack(anchor="w")

    def _open_bundled_doc(self, relative_path: str) -> None:
        path = resource_path(relative_path)
        if not path.exists():
            messagebox.showerror("Documentation not found", f"The bundled document could not be found:\n{path}", parent=self); return
        try:
